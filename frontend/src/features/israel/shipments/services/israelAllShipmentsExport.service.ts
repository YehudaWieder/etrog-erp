...
function formatDate(shippedAt: string | null): string {
  if (!shippedAt) return '—';
  return formatIsraelShipmentDate(new Date(shippedAt));
}

function buildHeader(labels: IsraelAllShipmentsTableLabels): string[] {
  return [
...
function buildDataRows(rows: IsraelShipmentRecord[], labels: IsraelAllShipmentsTableLabels): Array<string[]> {
  return rows.map((r) => [
...
export function printIsraelAllShipments({ lang, labels, rows, filterDisplayValues }: ExportParams): void {
...
export async function exportIsraelAllShipmentsToExcel({ lang, labels, rows, filterDisplayValues }: ExportParams): Promise<void> {
  if (typeof window === 'undefined') return;
  ...
  await downloadStyledExcel({
    ...
    fileName: `israel-all-shipments-${dateStamp}.xlsx`,
    header,
