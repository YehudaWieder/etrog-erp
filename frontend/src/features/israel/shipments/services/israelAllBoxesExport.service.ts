...
function buildHeader(labels: IsraelAllBoxesTableLabels): string[] {
  return [
    labels.colBoxNumber,
    labels.colField,
    labels.colShipmentNumber,
    labels.colItemsCount,
    labels.colStatus,
  ];
}

function buildDataRows(rows: IsraelBoxesTableRow[], labels: IsraelAllBoxesTableLabels): Array<string[]> {
  return rows.map((r) => [
    String(r.boxNumber),
    r.fieldName || '—',
...
export function printIsraelAllBoxes({ lang, labels, rows, filterDisplayValues }: ExportParams): void {
...
export async function exportIsraelAllBoxesToExcel({ lang, labels, rows, filterDisplayValues }: ExportParams): Promise<void> {
  if (typeof window === 'undefined') return;
  ...
  await downloadStyledExcel({
    ...
    fileName: `israel-all-boxes-${dateStamp}.xlsx`,
    header,
