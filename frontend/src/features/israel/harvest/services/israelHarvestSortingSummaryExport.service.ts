...
import type { CategoryGradeGroupSplit } from '../../../harvest/utils/gradeGroupBreakdown.util';
...
export type IsraelSortingSummaryExportRow = {
  key: string;
  label: string;
...
export type IsraelSortingSummaryExportMatrix = {
  title: string;
  rows: IsraelSortingSummaryExportRow[];
  grades: string[];
...
type ExportParams = {
  ...
  matrices: IsraelSortingSummaryExportMatrix[];
...
export function printIsraelHarvestSortingSummary({
  lang,
  labels,
  matrices,
  gradeGroupSplits,
...
export async function exportIsraelHarvestSortingSummaryToExcel({
  lang,
  labels,
  matrices,
  gradeGroupSplits,
  ...
  a.href = url;
  a.download = `israel-sorting-summary-${dateStamp}.xlsx`;
  a.click();
