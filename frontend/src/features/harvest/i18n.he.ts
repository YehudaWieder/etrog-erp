...
export const HARVEST_I18N_HE: HarvestI18n = {
  ...
  pageControls: {
    ...
    editHarvestDialog: {
      ...
      notesLabel: 'הערות',
      classifiedExceedsNetError: (classifiedTotal) => `הכמות הנטו לא יכולה לרדת מתחת לסה"כ המיונים (${classifiedTotal}).`,
      exactNetRequiresFullClassificationError: 'הכמות הנטו שווה לסה"כ המיונים. סמן כמיון מלא או הגדל את הכמות הנטו לפחות ב-1.',
      netIncreaseSwitchesToPartialWarning: 'הכמות הנטו גדולה מסה"כ המיונים. הקטיף יעבור בחזרה למיון חלקי.',
      markAsFullClassificationLabel: 'סמן כמיון מלא',
      confirm: 'שמירה',
      cancel: 'ביטול',
    },
    editSortingDialog: {
  ...
  formSubmission: {
    ...
    sortingTotalExceedsAvailable: (maxAllowed) => `סכום כמויות המיון לא יכול להיות גדול מ-${maxAllowed}.`,
    sortingTotalMustMatchAvailableForFullSorting: (requiredTotal) => `במיון מלא סכום כמויות המיון חייב להיות בדיוק ${requiredTotal}.`,
    sortingTotalWithClassifiedMustMatchAvailableForFullSorting: (requiredTotal) => `במיון מלא סה"כ המיון יחד עם סה"כ מויין חייב להיות בדיוק ${requiredTotal}.`,
    sortingTotalMustBeAtMostAvailableMinusOneForPartialSorting: (maximumTotal) => `במיון חלקי סה"כ המיון חייב להיות לכל היותר ${maximumTotal}.`,
