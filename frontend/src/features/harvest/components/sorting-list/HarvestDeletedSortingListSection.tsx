...
type HarvestDeletedSortingListSectionProps = {
  lang: 'he' | 'en';
  t: HarvestI18n;
...
export function HarvestDeletedSortingListSection({
  lang,
  t,
  filters,
  rows,
  ...
  selectedRowId,
  onRowClick,
}: HarvestDeletedSortingListSectionProps) {
  const sl = t.sortingList;
  const trash = t.sortingListTrash;
  ...
  return (
    <section className={`${workspaceStyles.workspace} ${panelStyles.workspace}`}>
      <header className={workspaceStyles.header}>
        <div>
          <p className={workspaceStyles.description}>{trash.description}</p>
        </div>
      </header>
      ...
      <div className={panelStyles.panelWide}>
        {isLoading ? <p className="seasons-manager__state">{trash.loading}</p> : null}

        {!isLoading && rows.length === 0 ? (
          <p className="seasons-manager__state">{trash.empty}</p>
        ) : null}
