import type { ShipmentRecord, ShipmentsTableLabels, ShipmentStatus } from '../shipments.types';
...
function formatDate(shippedAt: string | null): string {
  if (!shippedAt) return '—';
  return formatShipmentDate(new Date(shippedAt));
}

function buildHeader(labels: ShipmentsTableLabels): string[] {
  return [
...
function buildDataRows(rows: ShipmentRecord[], labels: ShipmentsTableLabels): Array<string[]> {
  return rows.map((r) => [
    String(r.shipmentNumber),
    String(r.totalBoxes),
    String(r.totalQuantity),
    labels.statusLabels[r.status as ShipmentStatus] ?? r.status,
...
function filtersHtml(
  ...
  if (filterDisplayValues.statusLabel) parts.push(`${esc(labels.statusFilterLabel)}: ${esc(filterDisplayValues.statusLabel)}`);
  if (parts.length === 0) return '';
  return `<div class="filters"><strong>${esc(lang === 'he' ? 'סינונים פעילים' : 'Active Filters')}:</strong>${parts.map((p) => `<div>${p}</div>`).join('')}</div>`;
}

export function printAllShipments({ lang, labels, rows, filterDisplayValues }: ExportParams): void {
...
export async function exportAllShipmentsToExcel({ lang, labels, rows, filterDisplayValues }: ExportParams): Promise<void> {
  if (typeof window === 'undefined') return;
  ...
  if (filterDisplayValues.statusLabel) filterRows.push([`${labels.statusFilterLabel}: ${filterDisplayValues.statusLabel}`, ...Array(header.length - 1).fill('')]);

  await downloadStyledExcel({
    sheetName: lang === 'he' ? 'כל המשלוחים' : 'All Shipments',
    fileName: `all-shipments-${dateStamp}.xlsx`,
    header,
