...
import { restoreShipmentItem } from '../../../services/shipmentItemsApi';
...
export function useRestoreShipmentItemDialog({ item, t, onSuccess }: Props): Result {
  ...
  const handleConfirm = useCallback(async () => {
    ...
    setError(null);
    try {
      await restoreShipmentItem(item.id, { suppressGlobalFeedback: true });
      setIsOpen(false);
      onSuccess();
    } catch (err) {
