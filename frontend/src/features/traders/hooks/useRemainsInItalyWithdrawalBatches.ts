...
import {
  fetchRemainsInItalyWithdrawalBatches,
  type RemainsInItalyWithdrawalBatch,
} from '../../../services/inventoryMovementsApi';

export function useRemainsInItalyWithdrawalBatches(
  enabled: boolean,
  filters: { seasonId: number | null; traderCategoryId?: number; grade?: string },
) {
  const [batches, setBatches] = useState<RemainsInItalyWithdrawalBatch[]>([]);
  ...
  useEffect(() => {
    ...
    fetchRemainsInItalyWithdrawalBatches({
      ...
      .catch((nextError: unknown) => {
        if (!isActive) return;
        setBatches([]);
        setError(nextError instanceof Error ? nextError.message : 'Failed to load remains-in-Italy withdrawals.');
      })
      .finally(() => {
        if (!isActive) return;
        setIsLoading(false);
