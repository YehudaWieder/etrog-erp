import type { TraderInventorySummaryMatrix } from '../utils/traderInventorySummaryMatrix.util';
import type { TraderInventoryI18n } from '../i18n';
...
export async function exportTraderInventoryExcel({
  fileName,
  sheetName,
  filterRows,
  summaryMatrix,
  ...
  summaryMatrix: TraderInventorySummaryMatrix;
  ...
  addHeaderRow([labels.columns.category, pitamWith, pitamWithout, pitamMixed, totalLabel]);

  summaryMatrix.categories.forEach((cat, i) => {
    addDataRow(makeDataCells(
      cat.label,
      [cat.totalsByPitamStatus.WITH_PITAM, cat.totalsByPitamStatus.WITHOUT_PITAM, cat.totalsByPitamStatus.MIXED],
      cat.total,
      i,
    ));
  });
  ...
  for (const cat of summaryMatrix.categories) {
    addEmptyRow();
    addCategoryNameRow(cat.label);
    addHeaderRow([gradeLabel, pitamWith, pitamWithout, pitamMixed, totalLabel]);

    summaryMatrix.grades.forEach((grade, i) => {
      const cell = summaryMatrix.gradeValues[grade]?.[cat.key] ?? { WITH_PITAM: 0, WITHOUT_PITAM: 0, MIXED: 0 };
