...
export class DefaultTraderCategoryController {
  ...
  @ApiOperation({
    summary:
      'Get all default trader categories with their shares for approval display. Roles: OWNER, MANAGER, EDITOR.',
  })
  @ApiResponse({
    status: 200,
    description:
  ...
  @ApiOperation({
    summary:
      'Get one default trader category by ID with all trader shares and total. Roles: OWNER, MANAGER, EDITOR.',
  })
  @ApiParam({
    name: 'id',
    type: Number,
    description: 'Default trader category ID.',
  })
  @ApiResponse({
    status: 200,
    description:
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid shares payload or total percent is not exactly 100.',
  })
  @ApiResponse({
    status: 404,
    description: 'One or more traders were not found.',
  })
  @ApiResponse({ status: 409, description: 'Category name already exists.' })
  @Roles(Role.OWNER, Role.MANAGER)
  @Post('with-shares')
  ...
  @ApiOperation({
    summary:
      'Delete a default trader category and its default shares. Roles: OWNER, MANAGER.',
  })
  @ApiParam({
    name: 'id',
    type: Number,
  ...
  @ApiResponse({
    status: 200,
    description: 'Default trader category deleted successfully.',
  })
  @ApiResponse({
    status: 404,
    description: 'Default trader category not found.',
  ...
  @ApiResponse({
    status: 409,
    description: 'Share already exists for this trader in this category.',
  })
  @Roles(Role.OWNER, Role.MANAGER)
  @Post('shares')
  ...
  @ApiOperation({
    summary:
      'Remove a trader share from a default category. Roles: OWNER, MANAGER.',
  })
  @ApiParam({
    name: 'categoryId',
    type: Number,
  ...
  @ApiResponse({
    status: 200,
    description: 'Default share removed successfully.',
  })
  @ApiResponse({ status: 404, description: 'Share not found.' })
  @Roles(Role.OWNER, Role.MANAGER)
