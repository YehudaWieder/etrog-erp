...
export class ItemCleanupService {
  ...
  async purgeExpiredSoftDeleted() {
    ...
    const expired = await this.prisma.shipmentItem.findMany({
    ...
    await this.prisma.shipmentItem.deleteMany({ where: { id: { in: ids } } });

    this.logger.log(`Purged ${ids.length} expired soft-deleted shipment item(s) (older than ${SOFT_DELETE_TTL_DAYS} days).`);
  }
}
